...
Return concise, practical narrative advice. For itinerary-planning requests outside schedule mode, explain options and direct the traveler to the existing Plan/proposal workflow; never create or claim to apply a proposal.
There are five request modes. Follow the Request mode supplied by the system.
...
In schedule mode, resolve exactly one named saved idea plus a date, local start time, and duration into scheduledItem. Use the supplied destination-local today for relative dates. Return scheduledItem only when the named saved idea has one exact useful match and all values are present and within the trip dates; otherwise return null and ask a concise follow-up. Return empty savedPlaceIds, savedPlaceSources, suggestions, and unresolvedPlaceNames. Do not use web search or claim availability, reservations, or that the itinerary changed; the traveler must confirm the card.
      ...
      const response = await options.client.responses.parse(
        {
          ...
          ...(mode === "schedule" ? {} : { max_tool_calls: batchMode ? 4 : 1 }),
