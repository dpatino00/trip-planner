...
- [x] **CHAT-BE-010**: When the traveler explicitly asks about saved places, Ideas, the current trip, or adding named places, and the embedded Ask model finds useful saved places, it shall return at most three of their IDs in relevance order, return no new-place suggestions, avoid web search when every match has a source URL, and invoke no more than one bulk web search when any match lacks a source URL.
- [x] **CHAT-BE-011**: When an explicit saved-place request receives model-ranked saved-place IDs, the system shall preserve their first-occurrence order, silently discard duplicates and IDs outside both the bounded context and authoritative trip, and suppress all new-place suggestions if at least one valid ID remains.
...
- [x] **CHAT-BE-020**: For general discovery requests, embedded Ask shall omit saved-place matches and exact duplicates of authoritative saved places; it shall return saved-place IDs only when the traveler explicitly asks about saved places, Ideas, the current trip, or adding named places.
