...
- [x] **CHAT-DATA-005**: Each general-discovery suggestion shall contain a non-null HTTPS source URL, each explicit-addition suggestion may contain a nullable HTTPS source URL, and shared saved-place and Action schemas shall remain compatible with nullable source URLs.
...
- [x] **CHAT-API-009**: If a standard embedded Ask model request exceeds twenty seconds or an explicit-addition request exceeds forty-five seconds, then the system shall return status 504.
...
- [x] **CHAT-BE-001**: When generating a standard embedded Ask response, the system shall configure one low-context web-search call and a 1,600-token output cap, and when generating an explicit-addition response, it shall configure no more than four low-context web-search calls and a 5,000-token output cap, while every mode shall disable provider storage and configure no other tools.
...
- [x] **CHAT-BE-010**: When the traveler explicitly asks about saved places, Ideas, or the current trip without requesting additions and the model finds useful saved places, embedded Ask shall return at most three IDs in relevance order, return no new-place suggestions, avoid web search when every match has a source URL, and invoke no more than one bulk web search when any match lacks a source URL.
...
- [x] **CHAT-BE-013**: If a proposed new-place or saved-place source URL lacks an exact match in the current bounded web-search evidence, then embedded Ask shall discard that source candidate and shall retain the proposed new place without a source URL only in explicit-addition mode.
...
- [x] **CHAT-BE-020**: For general discovery requests, embedded Ask shall omit saved-place matches and exact duplicates of authoritative saved places, and it shall return saved-place IDs only for saved-place lookups or explicit-addition requests.
...
- [x] **CHAT-UI-007**: When an Ask response contains a new-place suggestion, the interface shall display a clickable “Learn more” action only when a validated HTTPS source exists and shall always display generated Apple Maps, Google Maps, and directions actions before the traveler adds it to the trip.
